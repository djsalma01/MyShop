...
if(isset($_POST["submit"])){
    ...
    //if( require("connect_db_admin.php")){
        ...
        $sth1=$bdd->prepare("select id from categories where name='$categories';");
        $sth1->execute();
        $idc=$sth1->fetchAll(PDO::FETCH_OBJ);
        $idc=$idc[0]->id;
        
        $sth = $bdd->prepare("UPDATE products SET name='$name', price='$price', category_id='$idc', image='$image' WHERE id='$id';");
        $sth->execute();
